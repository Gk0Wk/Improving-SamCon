...
class Trajectory:
    ...
    def get_elite_inds(self, i):
        return np.array(self.elite_inds[i], dtype=int)

    # ===========================================================
    # get all samples (num = nSample)
    def get_prev_uid(self, i):
        return np.array(self.prev_uid[i], dtype=int)

    def get_curr_uid(self, i):
        return np.array(self.curr_uid[i], dtype=int)
    ...
    def find_path_and_save(self, cfg):
        ...
        num_path: int = paths.shape[0]

        paths_my_sys = paths.copy()
        for i in range(1, self._nIter + 1):
            candidates = self.get_curr_uid_elite(i)
            # idx_my_sys = np.array()
            idx_my_sys = np.array([np.where(candidates == paths[j, i])[0][0] for j in range(num_path)])
